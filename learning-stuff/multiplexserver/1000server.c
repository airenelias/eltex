...
int main()
{
	...
	struct epoll_event *revents = malloc(sizeof(struct epoll_event));
	struct epoll_event event;
	event.events = EPOLLIN;
	event.data.fd = sfd[0];
	epoll_ctl(epoll_fd, EPOLL_CTL_ADD, sfd[0], &event);
	event.events = EPOLLIN | EPOLLET;
	event.data.fd = sfd[1];
	epoll_ctl(epoll_fd, EPOLL_CTL_ADD, sfd[1], &event);
	while(1)
	{
		epoll_wait(epoll_fd, &event, 1, -1);
		if(event.events == EPOLLIN && event.data.fd == sfd[0])
		{
			socklen_t addrlen = sizeof(clientsockaddr);
		...
		if(event.events == EPOLLIN && event.data.fd == sfd[1])
		{
			socklen_t addrlen = sizeof(clientsockaddr);
			sockcount++;
